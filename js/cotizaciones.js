...
function cargarEstados() {
  ...
  const estados = [...new Set(cotizaciones.map(c => c.estado).filter(Boolean))].sort();
...
function aplicarFiltros() {
  ...
  filtradas = cotizaciones.filter(c => {
    ...
    if (texto) {
      ...
      const enNumero = normalizar(c.numero).includes(texto);
...
function render() {
  ...
  if (filtradas.length === 0) {
    ...
    lista.innerHTML = `
      <tr>
        <td colspan="7" class="cotizacion-vacia">
          <div>🧾</div>
          <strong>${cotizaciones.length ? "No hay cotizaciones con esos filtros" : "Aún no hay cotizaciones"}</strong>
          <span>${cotizaciones.length ? "Prueba con otra búsqueda." : "Crea la primera desde Nueva cotización."}</span>
        </td>
      </tr>`;
  ...
  lista.innerHTML = filtradas
    ...
    .map(c => {
      ...
      const url = `vista-cotizacion.html?numero=${encodeURIComponent(c.numero)}`;

      return `
        <tr class="historial-fila" data-url="${url}">
          <td data-label="Número"><strong>${escaparHTML(c.numero)}</strong></td>
          ...
          <td data-label="Estado">
            <span class="estado-chip ${claseEstado(c.estado)}">${escaparHTML(c.estado || "—")}</span>
          </td>
...
// Toda la fila abre la cotización
